...
# LP changed from AH path to LP computer path - copy + paste old code here
...
herps_wide <- herps %>%
  select(-c(Type, SurveyDate))%>%
  # add values_fill argument to fill NAs with 0's
  pivot_wider(names_from = Species, values_from = Count, values_fill = 0)
...
pal=pnw_palette("Sailboat", 1, type = "discrete")
```
...
dewpt <- ggplot(climate,
 ...
 # merge plots using facet
 (dewpt + temp)/(hum + precip)
 
 # double check that all plots with lines shown are significant
...
ggplot(herps%>%filter(Type=="Lizard"|Type=="LIZARD"), aes(x=Species, y=Count, fill=Species))+
  geom_col()+
  theme_classic()+
  ggtitle("Lizard abundance by species")
...
# this helps check assumptions - run in console and hit enter in the console after running to see plots pop up in the plot window
plot(lm_UTST_dewpt)
...
lm_UTST_dewpt2 <- lm(Count~dailydewpt, data= herps_climate %>% 
                                             filter(Species=="UTST" & Count <= 9))
summary(lm_UTST_dewpt2)


# GAM - don't recommend this for this particular data; the best case to use it is when you have roller-coaster type data
gam_UTST_dewpt <- gam(Count ~ s(dailydewpt, fx = FALSE, k = -1, bs = 'cr'), data = herps_climate %>% filter(Species == 'UTST'))

summary(gam_UTST_dewpt)

# plot of GAM is just geom_smooth 
ggplot(herps_climate%>%filter(Species=="UTST"), aes(x=Count, y=dailydewpt))+
  geom_point()+
  geom_smooth()
...
ggplot(herps_climate%>%filter(Species=="SCOC"), aes(x=Count, y=dailydewpt))+
  geom_point()+
  geom_smooth(method="lm")
...
ggplot(herps_climate%>%filter(Species=="UTST"), aes(x=Count, y=dailydrybulb))+
  geom_point()+
  geom_smooth(method="lm")
...
ggplot(herps_climate%>%filter(Species=="SCOC"), aes(x=Count, y=dailyrelhum))+
  geom_point()+
  geom_smooth(method="lm")
...
herps_rich<-specnumber(herps_wide2[,-c(1:5)])

herps2<-ftable(SurveyYear ~ Species, data=herps)
```


# Lauren's wild code chunk
```{r}
...
View(herps_rich_year)

herps_rich_year <- as_tibble('richness' = specnumber(herps_wide3[,-1]), 'SurveyYear' = herps_wide3$SurveyYear)

# join to herp data (here we'll use UTST as an example b/c its summarized by year)

UTST_rich <- left_join(UTST, herps_rich_year, by = 'SurveyYear')
